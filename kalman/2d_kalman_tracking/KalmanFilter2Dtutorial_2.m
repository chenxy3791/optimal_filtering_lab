...
%   It seems 'angular rotation' is independent and redundant in this example. 
...
C=[1 0 0  0  ; ...
   0 1 0  0  ]; %define the output matrix -- Only [x,y,theta] are measured.
B=[0.5*Ts^2 0        ; ...
   0        0.5*Ts^2 ; ...
   Ts       0        ; ...
   0       Ts        ]; %define the input matrix

x0  =[0;0;0;0]; %define the initial conditions
sys =ss(A,B,eye(4),[],Ts); %define a system to generate true data
...
u     = [ux;uy]; % Each column represents one time instant.
...
for(i=2:1:length(t))
    ...
    Xest(:,i)= Xest(:,i)+K*([xm(i); ym(i)]-C*Xest(:,i)); %Correcting: estimating the state
    P        = (eye(4)-K*C)*P; %Correcting: estimating P
end
